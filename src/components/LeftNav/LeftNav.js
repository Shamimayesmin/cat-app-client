...
const LeftNav = ({setItem, item,}) => {
	...
	const handleIncrement = (cat) => {
        ...
        const updatedData = catName.map(item => item._id === cat._id ? updatedItem : item);
        ...
        <div className='border p-10'>
		        {
		            catName.map(cat =>
		            <p key={cat._id}>
		                <button onClick={() => handleIncrement(cat)}className='bg-info rounded fs-5 w-75'><Link className='text-decoration-none text-white'>{cat.categoryName}</Link>{cat.counter}</button>
		            </p>)
		        }
		   </div>
    );
